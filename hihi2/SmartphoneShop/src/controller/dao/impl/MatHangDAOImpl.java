...
public class MatHangDAOImpl extends RootDAO implements MatHangDAO {
    ...
    public void addMatHang(MatHang matHang) {
        String sql = "INSERT INTO MatHang(maMH, maDM, tenMH, hangMH, img, giaMH,manHinh, heDieuHanh, camera, cpu, ram, pin) VALUES(?,?,?,?,?,?,?,?,?,?,?,?);";
    ...
    public void editMatHang(MatHang matHang) {
        String sql = "UPDATE MatHang SET maDM = ?, tenMH = ?, hangMH = ?, img = ?, giaMH = ?, manHinh = ?, heDieuHanh = ?, camera = ?, cpu = ?, ram = ?, pin = ? WHERE maMH = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
