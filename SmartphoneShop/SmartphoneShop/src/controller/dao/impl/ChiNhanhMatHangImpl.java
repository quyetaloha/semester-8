...
public class ChiNhanhMatHangImpl extends RootDAO implements ChiNhanhMatHangDAO {
    ...
    public void add(int maCN, int maMH, int soLuong) {
        String sql = "INSERT INTO ChiNhanhMatHang(maCN, maMH, soLuong) VALUES(?,?,?);";
    ...
    public void edit(int maMH, int maCN, int soLuong) {
        String sql = "UPDATE ChiNhanhMatHang SET soLuong = ? WHERE maCN = ? AND maMH = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
    ...
    public void delete(int maCN, int maMH) {
        String sql = "DELETE FROM ChiNhanhMatHang WHERE maCN = ? AND maMH = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
            ppst.setInt(1, maCN);
    ...
    public ArrayList<ChiNhanh> getChiNhanh(int maMH) {
        ...
        String slq = "SELECT * FROM ChiNhanhMatHang WHERE maMH = ?;";
        conn = getJDBCConnection();
        try {
    ...
    public ArrayList<MatHang> getDSMatHangByCN(int maCN) {
        ...
        String sql = "SELECT * FROM ChiNhanhMatHang WHERE maCN = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
            ppst.setInt(1, maCN);
            rs = ppst.executeQuery();
            while (rs.next()) {
                MatHang matHang = matHangDAO.getMatHang(rs.getInt("maMH"));
    ...
    public void deleteMatHang(int maMH) {
        String sql = "DELETE FROM ChiNhanhMatHang WHERE maMH = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
            ppst.setInt(1, maMH);
    ...
    public MatHang getMatHang(int maMH) {
        String sql = "SELECT * FROM ChiNhanhMatHang WHERE maMH = ?;";
    ...
    public boolean checkMaMH(int maCN, int maMH) {
        String sql = "SELECT * FROM ChiNhanhMatHang WHERE maCN = ? AND maMH = ?;";
        try {
            conn = getJDBCConnection();
            ppst = conn.prepareStatement(sql);
            ppst.setInt(1, maCN);
