...
public class GiaoDichDAOImpl extends RootDAO implements GiaoDichDAO {
    ...
    public void addGiaoDich(Items items) {
        String sql = "INSERT INTO GiaoDich(maHD, maMH, soLuong) VALUES(?,?,?);";
    ...
    public void editGiaoDich(Items items) {
        String sql = "UPDATE GiaoDich SET soLuong = ? WHERE maHD = ? AND maMH = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
    ...
    public void delete(int maHD, int maMH) {
        String sql = "DELETE FROM GiaoDich WHERE maHD = ? AND maMH = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
            ppst.setInt(1, maHD);
    ...
    public void delete(int maHD) {
        String sql = "DELETE FROM GiaoDich WHERE maHD = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
            ppst.setInt(1, maHD);
    ...
    public ArrayList<Items> getMatHangByHoaDon(int maHD) {
        String sql = "SELECT * FROM GiaoDich WHERE maHD = ?;";
