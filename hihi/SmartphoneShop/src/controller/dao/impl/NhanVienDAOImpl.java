...
public class NhanVienDAOImpl extends RootDAO implements NhanhVienDAO {
    ...
    public void addNhanVien(NhanVien nhanVien) {
        String sql = "INSERT INTO NhanVien(maCN,chucVu, taikhoan, matkhau, hoTen, diaChi, sdt) VALUES(?,?,?,?,?,?,?);";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
            ppst.setInt(1, nhanVien.getMaCN());
            ppst.setString(2, nhanVien.getRole());
            ppst.setString(3, nhanVien.getUser());
            ppst.setString(4, nhanVien.getPass());
            ppst.setString(5, nhanVien.getName());
            ppst.setString(6, nhanVien.getDiaChi());
            ppst.setString(7, nhanVien.getPhone());
            ppst.executeUpdate();
            conn.close();
    ...
    public void editNhanVien(NhanVien nhanVien) {
        String sql = "UPDATE NhanVien SET maCN = ?, chucVu = ?, taikhoan = ?, matkhau = ?, hoTen = ?, diaChi = ?, sdt = ? WHERE maNV = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
            ppst.setInt(1, nhanVien.getMaCN());
    ...
    public NhanVien getNhanVien(int maNV) {
        ...
        try {
            ...
            while (rs.next()) {
               NhanVien nhanVien = new NhanVien(maNV, rs.getInt("maCN"), rs.getString("chucVu"), rs.getString("taikhoan"),
                        rs.getString("matkhau"), rs.getString("hoTen"), rs.getString("diaChi"), rs.getString("sdt"));
                return nhanVien;
            }
            conn.close();
        } catch (SQLException ex) {
    ...
    public ArrayList<NhanVien> getNhanVien(String name) {
        ...
        try {
            ...
            rs = ppst.executeQuery();
            while (rs.next()) {
               NhanVien nhanVien = new NhanVien(rs.getInt("maNV"), rs.getInt("maCN"), rs.getString("chucVu"),
                        rs.getString("taikhoan"), rs.getString("matkhau"), rs.getString("hoTen"), rs.getString("diaChi"), rs.getString("sdt"));
                list.add(nhanVien);
            }
            conn.close();
        } catch (SQLException ex) {
    ...
    public ArrayList<NhanVien> getALL() {
        ...
        try {
            ppst = conn.prepareStatement(sql);
            ppst.setString(1, "employee");
            rs = ppst.executeQuery();
            while (rs.next()) {
                NhanVien nhanVien = new NhanVien(rs.getInt("maNV"), rs.getInt("maCN"), rs.getString("chucVu"), rs.getString("taikhoan"),
                        rs.getString("matkhau"), rs.getString("hoTen"), rs.getString("diaChi"), rs.getString("sdt"));
                list.add(nhanVien);
            }
            conn.close();
