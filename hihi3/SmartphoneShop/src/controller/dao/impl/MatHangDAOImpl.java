...
public class MatHangDAOImpl extends RootDAO implements MatHangDAO {
    ...
    public void addMatHang(MatHang matHang) {
        String sql = "INSERT INTO Sach(maSach, maLS, tenSach, nhaXuatBan, img, giaBan,tacGia, quocGia, nguoiBienDich, kichThuoc, moTa, ngayXuatBan) VALUES(?,?,?,?,?,?,?,?,?,?,?,?);";
    ...
    public void editMatHang(MatHang matHang) {
        String sql = "UPDATE Sach SET maLS = ?, tenSach = ?, nhaXuatBan = ?, img = ?, giaBan = ?, tacGia = ?, quocGia = ?, nguoiBienDich = ?, kichThuoc = ?, moTa = ?, ngayXuatBan = ? WHERE maSach = ?;";
        conn = getJDBCConnection();
        try {
            ppst = conn.prepareStatement(sql);
